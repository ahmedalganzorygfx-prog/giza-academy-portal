...
st.markdown("""
    <style>
    /* إخفاء زر التحميل العائم فقط من شريط أدوات العناصر */
    ...
    @media print {
        [data-testid="stSidebar"], .stButton, .stTextInput, header, footer {
            display: none !important;
        }
        body {
...
# 2. تحميل البيانات مع التخزين المؤقت الفائق السرعة (Cache)
@st.cache_data(show_spinner="جاري تحميل وتجهيز البيانات، يرجى الانتظار...")
def load_data():
    try:
        # إذا كان ملف الـ Excel كبير جداً، يفضل تحويله إلى CSV لتسريع القراءة فوراً
...
# استدعاء دالة التحميل المخزنة مؤقتاً
...
if selected_option == "🏠 الرئيسية":
    ...
    st.info("تم تفعيل ميزة التخزين المؤقت للبيانات (Caching) لتسريع التصفح والانتقال الفوري بين الأقسام دون إعادة التحميل.")
